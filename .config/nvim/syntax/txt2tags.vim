...
if &background == "light"
" ############ LIGHT ############
	hi default t2t_Link    ctermfg=Blue    guifg=#0930DE 
	hi default t2tBar         term=bold        cterm=bold        gui=bold
	hi default t2tBold        term=bold        cterm=bold        gui=bold		ctermfg=Blue    guifg=#445675
	hi default t2tItalic      term=italic      cterm=italic      gui=italic     ctermfg=DarkRed    guifg=#653A39
	hi default t2tStrike      term=italic      cterm=italic      gui=undercurl guisp=fg  ctermfg=Magenta    guifg=#644A9B
	hi default t2tUnderline   term=underline   cterm=underline   gui=underline     ctermfg=Green    guifg=#386742
	hi default t2tQuote       term=reverse     cterm=reverse     gui=reverse		ctermfg=Yellow    guifg=#DCC9A9
	hi default t2tTableAlign  term=reverse     cterm=reverse     gui=reverse
	hi default t2tComment     ctermfg=gray    guifg=#676767
	hi default t2tTitle    term=bold      cterm=bold      gui=bold     ctermfg=White    guifg=#E2ECFB    ctermbg=Red    guibg=#990000
	hi default t2tBlank       ctermbg=gray    guibg=#DCDCA9  
else
" ############ DARK ############
	hi default t2t_Link    ctermfg=cyan    guifg=#21A7FF
	hi default t2tBar         term=bold        cterm=bold        gui=bold
	hi default t2tBold        term=bold        cterm=bold        gui=bold		ctermfg=Blue    guifg=#BBCFF3
	hi default t2tItalic      term=italic      cterm=italic      gui=italic     ctermfg=Magenta     guifg=#EAA6A5
	hi default t2tStrike      term=italic      cterm=italic      gui=undercurl guisp=fg  ctermfg=Red   guifg=#C9B1FC
	hi default t2tUnderline   term=underline   cterm=underline   gui=underline     ctermfg=Green    guifg=#AEF7BD
	hi default t2tQuote       term=reverse     cterm=reverse     gui=reverse		ctermfg=Brown    guifg=#5B4625
	hi default t2tTableAlign  term=reverse     cterm=reverse     gui=reverse
	hi default t2tComment     ctermfg=brown    guifg=bisque
	hi default t2tTitle    term=bold      cterm=bold      gui=bold     ctermfg=White    guifg=#E2ECFB   ctermbg=Red    guibg=#990000 
	hi default t2tBlank       ctermbg=gray    guibg=#555555  	
endif
...
hi default t2tNumTitle      term=bold      cterm=bold      gui=bold     ctermfg=Red    guifg=#990000    
" comment the following line to avoid having trailing whitespaces in grey
...
hi default t2tPre  ctermfg=gray    guifg=#848B00  
hi default t2tPost  ctermfg=gray    guifg=#C05600  
